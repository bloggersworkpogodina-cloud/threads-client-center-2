    ...
    async def save_posts(self, client_id: int, post_date: str, posts: list[dict[str, str]]) -> list[aiosqlite.Row]:
        async with self.connect() as conn:
            for idx, post in enumerate(posts):
                await conn.execute(
                    """INSERT OR IGNORE INTO daily_posts(client_id, post_date, slot, body, image_url, source_row, sent_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?)""",
                    (
                        client_id,
                        post_date,
                        post.get("time"),
                        post["text"],
                        (post.get("image") or "").strip() or None,
                        int(post.get("source_row", idx)),
