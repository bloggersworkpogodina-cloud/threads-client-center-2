...
def generate_contract_pdf(client, signer_name: str, settings, output_path: str, draft: bool = False) -> str:
    ...
    story = []
    label = "ЧЕРНОВИК - ДЛЯ ПРОВЕРКИ" if draft else "ДОГОВОР ОКАЗАНИЯ УСЛУГ"
    ...
    story.append(_p("2. Услуги", h))
    story.append(_p(
        f"Исполнитель оказывает Заказчику следующие услуги:\n{client['services'] or 'Услуги не указаны.'}",
        body,
    ))
    story.append(_p(
    ...
    story.append(_p(
        f"Стоимость услуг составляет {money(client['service_price'])} за один месяц оказания услуг. "
        "Если стороны письменно в Telegram согласовали иной порядок оплаты для отдельного периода, "
        "применяется такое согласование.",
        body,
    ))

    story.append(_p("4. Порядок оказания услуг", h))
    story.append(_p(
    ...
    story.append(_p("6. Срок и прекращение", h))
    story.append(_p(
        "Договор действует с момента его акцепта Заказчиком в Telegram-боте и до прекращения сторонами. "
        "Услуги оказываются помесячно. Каждая сторона вправе отказаться от дальнейшего исполнения договора "
        "с учетом обязательных требований законодательства и оплаты фактически оказанных услуг и расходов.",
        body,
    ))
